...
public class PlayerPrivateDataClassConverter {
	public static void main(String[] args) throws IOException {
		...
		for (int x = 0; x < list.size();){
			if (!list.get(x).startsWith("private")){
				x++;
				continue;
			}
			if (!list.get(x).equalsIgnoreCase("")){
				...
				builder.append(" * @type ").append(convertWebsiteTypes(type)).append("\n");
		...
		bufferedWriter.newLine();
		bufferedWriter.newLine();
		bufferedWriter.write("PlayerPrivateDataField(String packetForm){");
		bufferedWriter.newLine();
